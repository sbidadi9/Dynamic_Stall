...
# Inputs
frequency_edge = 3.58 #3.31
frequency_flap = 3.58 #2.86

edge_amp_m = -0.086 #0.38
flap_amp_m = -0.00189 #1.27 

#rot_avg_deg = 90
rot_amp_deg = 1.43 #10.0
frequency_rot = 3.58 #2.38
...
def generate_mixed_motion(rot_avg_deg, theta_deg_array, x_offset_array, y_offset_array):
    ...
    # Plot to verify
#    plt.figure()
#    plt.plot(time_array, x_offset_array, "r", label="x offset (m)")
#    plt.plot(time_array, y_offset_array, "b", label="y offset (m)")
#    plt.plot(time_array, theta_deg_array, "g", label="theta offset (m)")
#    plt.xlabel("time (s)")
...
#    plt.show()
    ...
    plt.xlabel("t/T")
    plt.ylabel("Mixed Displacement [330]")
    plt.ylim([-4.1, 4.1])
    plt.grid()
    plt.legend()
    plt.show()
