...
with PdfPages('cl_ffa_w3_211_negative_aoa_vs_time.pdf') as pfpgs:
    ...
    case_cl = np.zeros(np.size(aoa_n)) 
    case_aoa = np.zeros(np.size(aoa_n))

    case_cl_static, case_cd_static, case_cm_static = get_avg_data(af_type[j], aoa_n)
    ...
    for i, c in enumerate(aoa):
        ...
        if c < -28.0:
           ...
           plt.plot(case_aoa[k], case_cl[k], marker = 'o', markersize=9, color=colors[k], label=af_thick[j])
#           plt.scatter(alpha_tot[::10], cl_tot[::10], marker = '+', color=colors[k])
    ...
    plt.plot(case_aoa, case_cl_static, linestyle='solid', linewidth=3, color='black')
