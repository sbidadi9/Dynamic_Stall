...
rot_amp_deg = [3.69, 2.94, 2.46, 1.99, 1.43, 0.97, 0.49]
frequency_rot = [1.99, 2.44, 2.77, 3.07, 3.58, 3.95, 3.89]
...
#N_iters = [1880, 1530, 2700, 2440, 2090, 1900, 1920]
N_iters = [1880, 1530, 1350, 1220, 1050, 948, 962]



def get_case_data(i,j):
    return pd.read_csv('/scratch/sbidadi/Dynamic_Stall/final/nalu_runs/' + af_type[j] + '/aoa_' + str(aoa[i]) + '/' + af_type[j] + '_' + str(aoa[i]) + '_combined.dat', sep="\s+", skiprows=1,header=None, names=[ "Time","Fpx","Fpy","Fpz","Fvx","Fvy","Fvz","Mtx","Mty","Mtz","Y+min","Y+max"],dtype=float)
...
with PdfPages('cl_ffa_w3_211_negative_aoa_vs_time.pdf') as pfpgs:
    fig = plt.figure()

    j = 0
    k = 0

    case_cl = np.zeros(np.size(aoa_m)) 
    case_aoa = np.zeros(np.size(aoa_m))

    case_cl_static, case_cd_static, case_cm_static = get_avg_data(af_type[j], aoa_m)
    ...
    for i, c in enumerate(aoa):
        ...
        if c < 28.0:
           ...
           alpha_tot = c + rot_amp_deg[j] * np.sin(2 * np.pi * frequency_rot[j] * time_tot)

           case_cl[k] = np.average(cl_tot)
           plt.plot(case_aoa[k], case_cl_static[k], marker = '^', markersize=9, color='black', label=af_thick[j])
#           plt.plot(case_aoa[k], case_cl[k], marker = 'o', markersize=9, color='red', label=af_thick[j])
#           plt.scatter(alpha_tot[::10], cl_tot[::10], marker = '+', color=colors[k])
           ...
           plt.scatter([alpha_tot.iloc[0]], [cl_tot.iloc[0]], color='green', marker='s')
           plt.scatter([alpha_tot.iloc[-1]], [cl_tot.iloc[-1]], color='red', marker='s')
...
with PdfPages('cl_ffa_w3_241_negative_aoa_vs_time.pdf') as pfpgs:
    fig = plt.figure()
...
with PdfPages('cl_ffa_w3_270_negative_aoa_vs_time.pdf') as pfpgs:
    fig = plt.figure()
...
with PdfPages('cl_ffa_w3_301_negative_aoa_vs_time.pdf') as pfpgs:
    fig = plt.figure()
...
with PdfPages('cl_ffa_w3_330_negative_aoa_vs_time.pdf') as pfpgs:
    fig = plt.figure()
...
with PdfPages('cl_ffa_w3_360_negative_aoa_vs_time.pdf') as pfpgs:
    fig = plt.figure()
...
with PdfPages('cl_ffa_w3_500_negative_aoa_vs_time.pdf') as pfpgs:
    fig = plt.figure()
