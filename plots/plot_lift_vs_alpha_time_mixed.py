...
with PdfPages('cl_ffa_w3_211_positive_aoa_vs_time.pdf') as pfpgs:
    ...
    for i, c in enumerate(aoa):
        ...
        if c > 28.0:
           ...
           plt.plot(case_aoa[k], case_cl_static[k], marker = 'o', markersize=8, color='black', label=af_thick[j])
           plt.plot(case_aoa[k], case_cl[k], marker = 'o', markersize=8, color=colors[k], label=af_thick[j])
           plt.scatter(alpha_tot[::10], cl_tot[::10], marker = '+', color=colors[k])

           k = k + 1

    plt.plot(case_aoa, case_cl_static, linestyle='solid', linewidth=5, color='black')
